    ...
    <div className="container mt-4">
      <div className="d-flex justify-content-between align-items-center mb-2 ">
        ...
        <div className="d-flex">
          <input
            type="text"
            className="form-control me-2 bg-light form fa-solid fa-search " style={{ width: '320px', height: '30px' } }
            placeholder="Search transactions"
          />
          <button onClick={fetchCustomers} className="btn btn-primary btn-sm" style={{ width: '80px',}} >
            Refresh
          </button>
          <button className="btn ms-2 btn-sm ">
            View All
          </button>
        </div>
      </div>
      {message && <div className="alert alert-success">{message}</div>}
      <table className="table" style={{borderBottom: "1px solid rgb(243, 240, 240)"}}>
        <thead>
          <tr>
            <th className="text-start" style={tableStyle}>Customer Name</th>
            <th className="text-start" style={tableStyle}>Email</th>
            <th className="text-start"  style={tableStyle}>Loan Type</th>
            <th className="text-start"  style={tableStyle}>Outstanding Blc</th>
            <th className="text-start"  style={tableStyle}>Status</th>
            <th className="text-start"  style={tableStyle}>Action</th>
          </tr>
        </thead>
        <tbody>
          {loading ? (
            ...
            customers.map((customer) => (
              <tr key={customer.id}>
                <td className="text-start">{customer.firstname} {customer.lastname}</td>
                <td className="text-start">{customer.email}</td>
                <td className="text-start" >{customer.loanType === "female" ? "paid" : "Ongoing"}</td>
                <td className="text-start">{customer.gender === "female" ? "N10.000" : "N20.000"}</td>
      ...
      {!loading && customers.length === 0 && (
        <div className="alert alert-info text-center">
          No customers found.{' '}
          <button onClick={fetchCustomers} className="btn btn-sm btn-primary ms-2">
            Refresh
          </button>
          </div>
        )}
